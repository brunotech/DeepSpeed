...
def model_parallel_cuda_manual_seed(seed):
    ...
    if torch.distributed.get_rank() == 0:
        logger.info(
            f'> initializing model parallel cuda seeds on global rank {torch.distributed.get_rank()}, model parallel rank {tp_rank}, and data parallel rank {mpu.get_data_parallel_rank()} with model parallel seed: {model_parallel_seed} and data parallel seed: {data_parallel_seed}'
        )
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, all_outputs, *args):
        ...
        if torch.is_tensor(outputs):
            non_grad_outputs = [] if outputs.is_floating_point() else [outputs]
        else:
            non_grad_outputs = [
